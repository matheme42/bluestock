...
///
/// When a [controllers] is specified, its [TextEditingController.text]
/// defines the [initialValue]. If this [FormField] is part of a scrolling
/// container that lazily constructs its children, like a [ListView] or a
/// [CustomScrollView], then a [controllers] should be specified.
/// The controllers's lifetime should be managed by a stateful widget ancestor
/// of the scrolling container.
///
/// If a [controllers] is not specified, [initialValue] can be used to give
/// the automatically generated controllers an initial value.
///
...
///
/// ```functions imports
/// import 'package:flutter/services.functions';
/// ```
///
/// ```functions
...
class SelectFormField extends FormField<String> {
  ...
  ///
  /// When a [controllers] is specified, [initialValue] must be null (the
  /// default). If [controllers] is null, then a [TextEditingController]
        ...
        super(
          ...
          builder: (FormFieldState<String> field) {
            ...
            Widget buildField(SelectFormFieldType peType) {
              late Future<void> Function() lfOnTap;
  ...
  void initValues() {
    if (_effectiveController?.text != null &&
      ...
      if (item!.isNotEmpty) {
        ...
        if (widget.changeIcon && item?['icon'] != null && item?['icon'] != '') {
          _icon = item?['icon'];
        }
      }
    }
...
class ItemPickerDialog extends StatefulWidget {
  ...
  @override
  ItemPickerDialogState createState() => ItemPickerDialogState();
}

class ItemPickerDialogState extends State<ItemPickerDialog> {
  final TextEditingController _oCtrlSearchQuery = TextEditingController();
  final List<Map<String, dynamic>> _lItemListShow = <Map<String, dynamic>>[];
  final List<Map<String, dynamic>> _lItemListOriginal =
      <Map<String, dynamic>>[];
  int _iQtItems = -1;
